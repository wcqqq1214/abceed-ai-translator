...
function makeRecoveryRequest(texts, model, reason) {
  ...
  request.body.messages[0].content += ` 上一次输出未通过校验：${reason} 错误提示中的残留片段仅用于定位，不是需要保留的原文。请从原文重新翻译残留所在的完整句子，并检查所有其他句子；结合上下文翻译所有日文引用，不要只修改报告的第一处残留，不要复述错误提示。日文中的人名、品牌名和片假名请用中文译名或中文音译，不能转写成拉丁字母，也不能残留假名。数字占位符也必须保留，不可改成汉字数字、合并或省略重复数字；例如“1杯1杯”中的两个占位符都要保留。提交前检查引号和括号内外均无日文假名残留，并逐个核对占位符数量及顺序。`;
  return request;
}
...
(() => {
  ...
  el('span', 'v1.11.4', updateGroup, 'version');
  ...
  checkUpdate.onclick = async () => {
    ...
    try {
      const result = await checkForUpdate(GM_xmlhttpRequest, '1.11.4');
      if (result.available) {
