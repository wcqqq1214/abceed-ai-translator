...
弹窗的 ↻ 可跳过当前缓存重新翻译，失败保留旧译文。译文本地缓存不自动过期，容量满时淘汰旧条目。菜单支持清除缓存、检查更新和重试失败内容。新内容会发送给配置的 AI 服务商，可能产生费用。

双击图片识别并翻译日文，保留英文；需当前模型支持图片输入，不支持时弹窗会提示，失败可重试。图片识别和译文均缓存，原图保持不变。

脚本未运行时，检查 [用户脚本执行权限](https://www.tampermonkey.net/faq.php?locale=zh&q=Q209)。
